...
artinya untuk arsitektur ABELINK (tesis: **model fixed, architecture changes**).

---

## 1. Peta model per kategori (yang relevan untuk ABELINK)

| Kategori | Model terbaik | Skor kunci | Implikasi untuk ABELINK |
|---|---|---|---|
| Terminal / agentic coding | Fable 5.1 (55.8% TB 4.0; Mythos 5.1 60.9%), GLM-5.3 (open-weights terkuat), GPT-6 Astra (57.9%) | TB 4.0 | AbelinkBench TB 4.0 core set valid sebagai acuan; model coding `custom` endpoint (GLM/Qwen/DeepSeek open) sudah level frontier |
| Computer use | Fable 5.1: 77.9% (partial) / 41.7% (strict) OSWorld 2.0; Astra 72.6% | OSWorld 2.0 | ABELINK harus punya benchmark OS-control sendiri (AbelinkBench pilar OS) — xdotool path kita bisa diverifier deterministik |
...
| Browser | Astra & Fable via WebArena/WebVoyager | — | Fase C3 extension bridge adalah prasyarat benchmark browser ABELINK |
| Research/long-horizon | Kimi K3 (BrowseComp 91.2, 1M ctx, open), Astra (1.05M ctx, effort `max`) | BrowseComp, HLE | Sub-agent pipeline ABELINK cocok: relay + wait_subagents = pola swarm; K3 open-weight bisa jadi model riset lokal |
...
| Cyber | Gemini 3.8 Flash Cyber (Fairwind), GLM-5.3 (emergent cyber), Mythos 5.1 | CyberGym, CWE-Bench | MODEL TIDAK dipakai untuk offensive di ABELINK; pola safeguards yang bisa dicontek: akses berlapis (Lihat §3) |
| Vision | DeepSeek-V4-Flash-Vision-Exp, Gemini 3.8 multimodal | — | Kamera/vision ABELINK bisa pakai ini via custom endpoint |

## 2. Pola lintas vendor yang bisa langsung diterapkan ke ABELINK
...
1. **Effort levels (Fable 5.1, Astra, Gemini 3.8)**
   ...
   → **ABELINK menerapkan:** `effortLevel` (low/medium/high) di Configuration →
...
2. **Cache reads menentukan biaya agentic (Anthropic)**
   ...
   → ABELINK: prompt ABELINK stabil (persona/skills) di depan, observasi
...
3. **"Works harder" sebagai desain (Gemini 3.8)**
   ...
   langkah murah.
   → Ini validasi tesis ABELINK: no-turn-limit sub-agent + retry + verification
...
4. **Safeguards berlapis berdasarkan domain (Mythos vs Fable, Flash Cyber Fairwind)**
   ...
   trusted-access. Pola untuk ABELINK: graduated approval per family (sudah ada)
...
5. **Long-running readability (Jane Street quote)**
   ...
   → ABELINK: intermediate_answer + execution steps HUD (SubagentIntercom)
...
6. **Open-weights gap menutup (GLM-5.3, Kimi K3, Qwen, DeepSeek)**
   ...
   → ABELINK harus first-class di LM Studio / custom endpoint (sudah) dan
   benchmark-nya harus bisa jalan pada model lokal (AbelinkBench adapter sudah
   provider-agnostic via sidecar).

## 3. Pemetaan ke AbelinkBench (matrix existing diperbarui)
...
- OSWorld 2.0: strict 36-42% di vendor terbaik → benchmark OS ABELINK harus
...
browser (Fase C3) → ABELINK-Eval. Tambahan: laporkan skor **per effort level**
(low/medium/high) di report AbelinkBench agar perbandingan architecture-vs-model
adil.

## 4. Rekomendasi model default ABELINK (per use case)
