...
**Keywords:** PR2, long-horizon, effort budget, effortSystem, MAX_PLAN_STEPS, trajectory lineage, trajLineage, taskStore, taskExecutor, durable resume, state leak, pause restart, strategy trace, trajectory supervisor, nextStrategy, AbelinkBench, arch axis.
...
|---|---|---|---|---|
| Hardcoded step budget 25 | `src/hooks/agent/useAbelinkPlan.js` | `MAX_PLAN_STEPS = 25` statis memotong task kompleks sebelum konvergen | Dibuat `resolvePlanStepBudget()` berbasis `effortSystem` + `trajLineage.js` menerima `maxAttempts` dinamis | ✅ 8 unit tests passed |
...
| ReAct resume pointer kacau | `src/api/taskStore.js` & `useAbelinkPlan.js` | `resumeAgentTask` tidak memuat step utuh; `useAbelinkPlan` tidak menyambungkan durable taskId | Return task with steps + resume handler di `useAbelinkPlan` via `opts.resumeTaskId` | ✅ Verified offline |
...
- `src/hooks/agent/useAbelinkPlan.js`: dynamic budget, lineage budget injection, durable task resume handler.
