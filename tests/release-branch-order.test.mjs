...
const setupRepo = () => {
  ...
  fs.writeFileSync(path.join(TEMP_DIR, 'package.json'), JSON.stringify({ name: 'abelink', version: '1.0.0-alpha.1' }, null, 2))
  fs.writeFileSync(path.join(TEMP_DIR, 'src-tauri/Cargo.toml'), '[package]\nversion = "1.0.0-alpha.1"')
  fs.writeFileSync(path.join(TEMP_DIR, 'README.md'), 'ABELINK Linux')
  run('git add -A')
