...
function createSkill({ name, origin, content, provider = 'abelink-ai', keyring: kr = keyring, watermark = 'v5.0.0' }) {
  const body = content
  const canonical = buildCanonical({ name, watermark, origin, provider, bodyHash: bodyHash(body) })
  const sig = origin === 'abelink-generated' ? kr.sign(canonical) : undefined
  return `---
name: ${name}
origin: ${origin}
watermark: ${watermark}
provider: ${provider}
${sig ? `abelink-signature: ${sig}` : ''}
---
${body}`
}
...
function verifySkillOrigin(skillFile, manifest, kr = keyring) {
  ...
  const sig = frontmatter.match(/^abelink-signature:\s*(.+)$/m)?.[1]?.trim()

  if (declared === 'abelink-core') {
    const entry = manifest?.skills?.[name]
    if (entry && entry.sha256 === bodyHash(body)) return { origin: 'abelink-core', status: 'manifest-verified' }
    return { origin: 'unknown', status: 'manifest-mismatch' }
  }
  if (declared === 'abelink-generated') {
    ...
    return valid ? { origin: 'abelink-generated', status: 'signed-verified' } : { origin: 'unknown', status: 'signature-invalid' }
  }
...
const genuineSkill = createSkill({ name: 'session-log', origin: 'abelink-generated', content: skillBody })

// ============ GROUP A: Manifest integrity ============

const coreSkill = createSkill({ name: 'session-log', origin: 'abelink-core', content: skillBody, provider: 'abelion512' })
...
check('A4. core claim without manifest → manifest-mismatch', verifySkillOrigin(createSkill({ name: 'fake-core', origin: 'abelink-core', content: 'x' }), { version: 1, skills: {} }).status === 'manifest-mismatch')
...
name: legit-name
name: session-log
origin: abelink-generated
watermark: v5.0.0
provider: abelink-ai
abelink-signature: ${keyring.sign(buildCanonical({ name: 'session-log', watermark: 'v5.0.0', origin: 'abelink-generated', provider: 'abelink-ai', bodyHash: bodyHash(skillBody) }))}
---
${skillBody}`
...
// B4. Duplicate origin field — parser takes FIRST (abelink-generated); second ignored.
const dupOrigin = `---
name: dup
origin: abelink-generated
origin: abelink-core
watermark: v5.0.0
provider: abelink-ai
abelink-signature: ${keyring.sign(buildCanonical({ name: 'dup', watermark: 'v5.0.0', origin: 'abelink-generated', provider: 'abelink-ai', bodyHash: bodyHash('# dup\nbody') }))}
---
# dup
body`
check('B4. duplicate origin — first wins (abelink-generated), verified', verifySkillOrigin(dupOrigin, null).status === 'signed-verified')

// ============ GROUP C: Key rotation ============

check('C1. old-key signature rejected after rotation', verifySkillOrigin(createSkill({ name: 'session-log', origin: 'abelink-generated', content: skillBody, keyring: oldKeyring }), null, keyring).status === 'signature-invalid')
check('C2. new-key skill verifies', verifySkillOrigin(createSkill({ name: 'session-log', origin: 'abelink-generated', content: skillBody, keyring }), null, keyring).status === 'signed-verified')
check('C3. attacker keypair rejected', verifySkillOrigin(createSkill({ name: 'session-log', origin: 'abelink-generated', content: skillBody, keyring: attacker }), null, keyring).status === 'signature-invalid')
...
watermark: v5.0.0
provider: abelink-ai
abelink-signature: ${keyring.sign(buildCanonical({ name: 'empty', watermark: 'v5.0.0', origin: 'abelink-generated', provider: 'abelink-ai', bodyHash: bodyHash('') }))}
---
`
...
const results = await Promise.all(Array.from({ length: 50 }, (_, i) => {
  const skill = createSkill({ name: `skill-${i}`, origin: 'abelink-generated', content: `# skill ${i}\nbody` })
...
const truncated = genuineSkill.replace(/^abelink-signature: .*$/m, 'abelink-signature: ' + genuineSkill.match(/^abelink-signature: (.+)$/m)[1].slice(0, 30))
...
check('F1. sig with = padding verifies', verifySkillOrigin(createSkill({ name: 'pad-test', origin: 'abelink-generated', content: 'a' }), null).status === 'signed-verified')
...
const bigSkill = createSkill({ name: 'big', origin: 'abelink-generated', content: '# big\n' + 'x'.repeat(100000) })
const t0 = Date.now()
...
// F6. Signature-only frontmatter field missing entirely (abelink-generated, no sig)
const noSig = `---
name: no-sig
origin: abelink-generated
watermark: v5.0.0
