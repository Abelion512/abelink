// Proof 2 of 2 — LIFECYCLE WITH STORAGE (post-merge audit PR #26).
// Full taskRuntime lifecycle under Bun with NO window, React, localStorage,
// or Tauri globals — through the REAL default store (taskStore.js -> Dexie).
// Persistence here comes from the same fake-indexeddb shim the vitest suite
// uses: this proves the boundary works with a compatible store, NOT that Bun
// ships one. Production Bun without a shim still fails fast (see proof 1:
// scripts/headless-task-runtime.mjs).
import assert from 'node:assert/strict'

for (const g of ['window', 'localStorage']) {
  if (g in globalThis) {
    console.error(`[headless-lifecycle] FAIL: globalThis.${g} must be absent`)
    process.exit(1)
  }
}
...
console.log('[headless-lifecycle] taskRuntime via real store + test shim: create/start/checkpoint/pause/resume/cancel/getResult OK (no window/React/Tauri)')
